...
// Add (union) interests for an actor. Returns the merged subscription. An optional
// `notify` URL turns on best-effort webhook push for this actor (see fanout); pass
// "none" to clear it.
export function setWatch(slug, actor, { tickets = [], epics = [], columns = [], mentions = false, notify } = {}) {
  safeActor(actor);
  ...
  else if (cur.notify) next.notify = cur.notify; // preserve across unioning updates
  all[actor] = next;
