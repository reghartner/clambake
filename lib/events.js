...
import { fanout } from "./watchers.js";
...
// Append an event, then fan it out to subscribers' inboxes. Best-effort: neither
// logging nor fan-out may break the underlying write, so failures are swallowed.
// A `ts` is stamped if the caller didn't supply one.
export function emitEvent(slug, event) {
  const full = { ts: new Date().toISOString(), ...event };
  try {
    fs.appendFileSync(eventsFile(slug), JSON.stringify(full) + "\n");
  } catch {}
  try {
    fanout(slug, full);
  } catch {}
...
// offset is the new cursor.
export function readEvents(slug, from = 0) {
  return readNdjson(eventsFile(slug), from);
}

// Core cursor reader for any append-only ndjson file (the event log and the per-actor
// inboxes both use it). Only whole lines are consumed; a partial trailing line (a
// concurrent append) is left for next time, so a reader never sees a half-written line.
export function readNdjson(file, from = 0) {
  let fd;
  try {
