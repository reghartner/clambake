...
export function createTicket(slug, fields = {}, actor = "ui") {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    try {
      ...
      emitEvent(slug, { type: "created", ticket: id, actor, status: t.status, title: t.title, epic: t.epic });
      return t;
    } catch (e) {
      if (e.code === "EEXIST") {
...
export function updateTicket(slug, id, patch = {}, actor = "ui") {
  ...
  if (editedFields.length) {
    emitEvent(slug, { type: "edited", ticket: id, actor, fields: editedFields, epic: t.epic });
  }
  return saved;
}
...
export function appendNote(slug, id, text, expectedUpdatedAt, actor = "ui") {
  ...
  t.updatedAt = stamp;
  const saved = writeTicket(slug, t);
  emitEvent(slug, { type: "noted", ticket: id, actor, epic: t.epic, summary: String(text).trim().slice(0, 140) });
  return saved;
}
