...
function safeSeg(seg, label = "value") {
  if (typeof seg !== "string" || !/^[A-Za-z0-9._-]+$/.test(seg)) {
...
export function createProject({ slug, name, idPrefix, staleDays, columns }) {
  safeSeg(slug, "project");
  ...
  const cfg = normalizeProject(slug, { name, idPrefix, staleDays, columns });
  fs.writeFileSync(
    path.join(dir, "project.json"),
    JSON.stringify({ name: cfg.name, idPrefix: cfg.idPrefix, staleDays: cfg.staleDays, columns: cfg.columns }, null, 2)
  );
  return cfg;
}
...
export function createTicket(slug, fields = {}, actor = "ui") {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    const t = normalizeTicket({ ...fields, id, ac: normalizeAc(fields.ac), lastActor: actor, createdAt: ts, updatedAt: ts }, fields.body || "");
    const file = path.join(ticketsDir(slug), `${id}.md`);
    try {
