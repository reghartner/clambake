...
import { extractMentions } from "./watchers.js";
// Re-export the subscription/inbox surface so the CLI's local backend matches the
// HTTP client's method set (cli.js calls store.setWatch / readInbox / … uniformly).
export { setWatch, unwatch, getWatch, getWatchers, readInbox, waitInbox } from "./watchers.js";
...
export function createTicket(slug, fields = {}, actor = "ui") {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    try {
      ...
      emitEvent(slug, { type: "created", ticket: id, actor, status: t.status, title: t.title, epic: t.epic, mentions: extractMentions(t.body) });
      return t;
    } catch (e) {
      if (e.code === "EEXIST") {
...
export function updateTicket(slug, id, patch = {}, actor = "ui") {
  ...
  t.lastActor = actor;
  t.updatedAt = nowIso();
  const saved = writeTicket(slug, t);
  // Emit a "moved" event for a column change and/or an "edited" event for other
  // field changes — a single update can do both (e.g. status + assignee).
  if (t.status !== prevStatus) {
    emitEvent(slug, { type: "moved", ticket: id, actor, from: prevStatus, to: t.status, epic: t.epic });
  }
  const editedFields = allowed.filter((k) => k in patch && k !== "status");
  if (editedFields.length) {
    // Mentions only count when the body itself changed (avoid re-pinging on every edit).
    const mentions = "body" in patch ? extractMentions(t.body) : [];
    emitEvent(slug, { type: "edited", ticket: id, actor, fields: editedFields, epic: t.epic, mentions });
  }
  return saved;
}
...
export function appendNote(slug, id, text, expectedUpdatedAt, actor = "ui") {
  ...
  t.updatedAt = stamp;
  const saved = writeTicket(slug, t);
  emitEvent(slug, { type: "noted", ticket: id, actor, epic: t.epic, summary: String(text).trim().slice(0, 140), mentions: extractMentions(text) });
  return saved;
}
...
export function addAttachment(slug, id, rawName, buffer, actor = "ui") {
  ...
  t.updatedAt = nowIso();
  writeTicket(slug, t);
  emitEvent(slug, { type: "attached", ticket: id, actor, epic: t.epic, summary: filename });
...
export function archiveTicket(slug, id) {
  ...
  emitEvent(slug, { type: "archived", ticket: id, actor: "system" });
  return { archived: id };
}
...
export function unarchiveTicket(slug, id) {
  ...
  emitEvent(slug, { type: "unarchived", ticket: id, actor: "system" });
