...
export const DEFAULT_PROJECT = {
  ...
  idPrefix: "TICK",
  staleDays: 5,
  // Auto-archive a ticket this many days after it entered "done". null = off
  // (the default — opt in per project so done columns don't empty unexpectedly).
  archiveDoneAfterDays: null,
...
export function normalizeProject(slug, raw = {}) {
  ...
  return {
    ...
    archiveDoneAfterDays: Number.isFinite(raw.archiveDoneAfterDays) ? raw.archiveDoneAfterDays : null,
    columns,
  };
}
...
export function normalizeTicket(data = {}, body = "") {
  return {
    ...
    doneAt: data.doneAt ?? null, // when the ticket entered "done"; drives auto-archive age
    body: body || "",
  };
}
...
// How many days a ticket has been finished. Ages from `doneAt` (set when it
// entered done), falling back to `updatedAt` for tickets created before doneAt
// existed — which is also what the very first sweep keys off. Returns null for
// non-done tickets or unparseable timestamps.
export function doneAgeDays(ticket, now = new Date()) {
  if (!DONE_COLUMNS.has(ticket.status)) return null;
  const stamp = ticket.doneAt || ticket.updatedAt;
  const ms = Date.parse(stamp);
  if (!Number.isFinite(ms)) return null;
  return (new Date(now).getTime() - ms) / 86_400_000;
}

// Whether a ticket is old enough in "done" to be auto-archived. `afterDays`
// null/negative => archiving disabled => never eligible.
export function shouldArchive(ticket, afterDays, now = new Date()) {
  if (!Number.isFinite(afterDays) || afterDays < 0) return false;
  const age = doneAgeDays(ticket, now);
