...
export function makeClient(base) {
  ...
  return {
    ...
    waitInbox(p, actor, { timeoutMs, peek } = {}) {
      const q = new URLSearchParams();
      if (Number.isFinite(timeoutMs)) q.set("timeout", String(timeoutMs));
      if (peek) q.set("peek", "1");
      const qs = q.toString();
      return req(base, "GET", `/api/projects/${seg(p)}/inbox/${seg(actor)}/wait${qs ? `?${qs}` : ""}`);
    },
